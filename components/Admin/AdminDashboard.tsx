...
const AdminDashboard: React.FC = () => {
  ...
  const [isEditing, setIsEditing] = useState<string | null>(null); // Stores the ID of the movie being edited
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (isAdding) {
        ...
        setEditedMovie(prev => prev ? { ...prev, imageUrl: blob.url } : null);
      }
    } catch (error) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editedMovie) {
      await db.updateMovie(editedMovie.id, editedMovie);
      setIsEditing(null);
      setEditedMovie(null);
      loadMovies();
    }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-24">
      <div className="flex justify-between items-center mb-12">
        ...
        <button 
          onClick={() => { setIsAdding(!isAdding); setIsEditing(null); setEditedMovie(null); }}
        ...
        >
          {isAdding ? 'CANCEL ADD' : 'ADD NEW MOVIE'}
        </button>
      </div>
      ...
      {isEditing && editedMovie && (
        <form onSubmit={handleUpdate} className="bg-white/5 border border-white/10 rounded-2xl p-8 mb-12 animate-in slide-in-from-top duration-300">
          <h2 className="text-2xl font-brand tracking-widest text-white mb-6">EDIT MOVIE: {editedMovie.title}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Movie Title</label>
              <input 
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  onChange={e => setEditedMovie({...editedMovie, imageUrl: e.target.value})}
                  className="flex-1 bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-500 outline-none"
                  placeholder="https://..."
                />
                <label className="cursor-pointer bg-white/10 hover:bg-white/20 border border-white/10 rounded-xl px-4 py-3 flex items-center justify-center transition-all">
                  <span className="text-xs font-bold uppercase tracking-widest">{uploading ? '...' : 'Swap Image'}</span>
                  <input type="file" className="hidden" accept="image/*" onChange={handleFileUpload} disabled={uploading} />
                </label>
              </div>
            </div>
            <div>
              ...
              <input 
                ...
                onChange={e => setEditedMovie({...editedMovie, year: parseInt(e.target.value)})}
                className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-500 outline-none"
              />
            </div>
            <div>
          ...
          <div className="flex gap-4">
            <button type="submit" className="flex-1 bg-white text-black font-bold py-4 rounded-xl hover:bg-gray-200 transition-colors">
              SAVE CHANGES
            </button>
            <button type="button" onClick={handleCancelEdit} className="flex-1 bg-white/10 text-white font-bold py-4 rounded-xl hover:bg-white/20 transition-colors">
              CANCEL
            </button>
          </div>
        </form>
